...
**The web app serves both roles as of Sprint 10.** It was originally specified as an internal moderation dashboard only; that was reversed deliberately so people can watch a circle from a desk. Both audiences live in one Next.js app (`apps/admin-web`, to be renamed) under separate layouts and separate cookies, backed by the two isolated auth systems in `docs/06-auth-flow.md` — a user session and an admin session remain different tokens against different tables. Raising an SOS stays mobile-only: the phone is the device you are carrying when you need it. See `docs/09-sprint-timeline.md` Sprints 10–12.
